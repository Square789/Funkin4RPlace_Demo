...
class WeekEditorFreeplayState extends MusicBeatState
{
	...
	var bg:FlxSprite;
	private var grpSongs:FlxTypedGroup<TitleCardFont>;
	...
	override function create() {
		...
		grpSongs = new FlxTypedGroup<TitleCardFont>();
		add(grpSongs);

		for (i in 0...weekFile.songs.length)
		{
			var songText:TitleCardFont = new TitleCardFont(0, (70 * i) + 30, weekFile.songs[i][0], true, false);
