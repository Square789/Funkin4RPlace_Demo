...
class NoteOffsetState extends MusicBeatState
{
	...
	var rating:FarpSprite;
	var comboNums:FlxTypedSpriteGroup<FarpSprite>;
	var dumbTexts:FlxTypedGroup<FlxText>;

	var barPercent:Float = 0;
	var delayMin:Int = 0;
	var delayMax:Int = 500;
	var timeBarBG:FarpSprite;
	var timeBar:FlxBar;
	var timeTxt:FlxText;
	var beatText:TitleCardFont;
	...
	override public function create()
	{
		...
		rating = new FarpSprite().loadUISkin('sick', 0.7, 5/6);
		...
		comboNums = new FlxTypedSpriteGroup<FarpSprite>();
		comboNums.cameras = [camHUD];
		add(comboNums);

		var seperatedScore:Array<Int> = /* Square */ [7, 8 ,9];

		var daLoop:Int = 0;
		for (i in seperatedScore)
		{
			var numScore:FarpSprite = new FarpSprite(43 * daLoop).loadUISkin('num$i', 0.5, 5/6);
		...
		beatText = new TitleCardFont(0, 0, 'Beat Hit!', true, false, 0.05, 0.6);
		beatText.x += 260;
		beatText.alpha = 0;
		...
		timeBarBG = new FarpSprite(0, timeTxt.y + 8).loadUISkin('timeBar', 1.2);
	...
	override public function update(elapsed:Float)
	{
		...
		if (onComboMenu)
		{
			...
			if (FlxG.mouse.justPressed)
			{
				...
				// @CoolingTool: probably no one will read this every cause who the fuck gives a shit about
				// "Adjust Delay and Combo" but for some reason the line below just didn't fucking exist 
				// and i had to copy and paste it from the og psych. how did this happen lmao
