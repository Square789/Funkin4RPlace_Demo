...
class FunkinLua {
	...
	public function new(script:String) {
		...
		set('ratingName', '');
		set('ratingFC', '');
		set('version', MainMenuF4rpState.psychEngineVersion.trim());
		set('versionExtra', MainMenuF4rpState.psychEngineExtraVersion.trim());
		...
		Lua_helper.add_callback(lua, "setTimeBarColors", function(leftHex:String, rightHex:String) {
			...
			// @CoolingTool: epic r/place hud fail
			// i've been wanting for a while to make place hud optional so maybe come back here?
		...
		Lua_helper.add_callback(lua, "playSound", function(sound:String, volume:Float = 1, ?tag:String = null) {
			if (tag != null && tag.length > 0) {
				...
				if (soundFile != null) {
					PlayState.instance.modchartSounds.set(tag, FlxG.sound.play(soundFile, volume, false, PlayState.instance.soundGroup, function() {
			...
			var soundFile = Paths.sound(sound);
			if (soundFile != null) {
				FlxG.sound.play(soundFile, volume, PlayState.instance.soundGroup);
				return true;
			}
