...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		while (i < modsList.length)
		{
			...
			newMod.alphabet = new TitleCardFont(0, 0, mods[i].name, true, false, 0.05);
			var scale:Float = Math.min(840 / newMod.alphabet.width, 1);
			newMod.alphabet = new TitleCardFont(0, 0, mods[i].name, true, false, 0.05, scale);
	...
	override function update(elapsed:Float)
	{
		...
		if (canExit && controls.BACK)
		{
			...
			saveTxt();
			if (needaReset) {
				// @Square789: I fail to see why this is here. why,.
				// Mod support is pried out regardless, so probably doesn't matter,
				// but this is referencing deleted states, so i commented it out.
...
class ModMetadata
{
	...
	public var alphabet:TitleCardFont;
